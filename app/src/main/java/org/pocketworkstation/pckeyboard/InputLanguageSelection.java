    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            getSupportFragmentManager()
                    .beginTransaction()
                    .replace(R.id.settings_container, new SelectionFragment())
                    .commit();
        }
    }

    public static class SelectionFragment extends PreferenceFragmentCompat {
        ...
        private boolean hasDictionary(Locale locale) {
            ...
            // We removed binary dictionary native code, so size is always 0.
            // But we keep the logic to not break compilation, assuming BinaryDictionary stub exists.
            BinaryDictionary bd = new BinaryDictionary(getActivity(), dictionaries, Suggest.DIC_MAIN);

            // Stubbed check - since we removed native dictionary support, this will likely be false
            if (bd.getSize() > Suggest.LARGE_DICTIONARY_THRESHOLD / 4) {
                haveDictionary = true;
            } else {
                BinaryDictionary plug = PluginManager.getDictionary(getActivity().getApplicationContext(), locale.getLanguage());
                if (plug != null) {
                    bd.close();
                    bd = plug;
                    haveDictionary = true;
